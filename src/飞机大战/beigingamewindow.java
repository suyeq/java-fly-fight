...
import javax.swing.*;
public class beigingamewindow {//游戏开始的选项
	...
	AudioClip audio = Applet.newAudioClip(gamePanel.c4);//选择时的音乐
	public void beginwindow() {
		frame1.setLayout(null);//设置空布局
		frame1.setSize(450, 670);//设置框的大小
		...
		Font font1 = new Font("华文行楷", Font.BOLD, 20);//设置字体
		...
		frame1.add(alabel3);
		frame1.add(alabel4);
		alabel1.addMouseListener(myAction1);//对每项进行监听
		alabel2.addMouseListener(myAction1);
		alabel3.addMouseListener(myAction1);
		alabel4.addMouseListener(myAction1);
		frame1.addWindowListener(new WindowAdapter() {//监听匿名类
			public void windowClosing(WindowEvent e) {//该方法是判断是否推出
	...
	class MyMouseListener implements MouseListener {//实现监听类，监听鼠标的变化
		public void mouseClicked(MouseEvent arg0) {//该方法判断鼠标是否点击
		}

		public void mouseEntered(MouseEvent arg0) {//该方法判断是否鼠标进入某项事物
			audio.play();
		...
		public void mouseExited(MouseEvent arg0) {//判断鼠标是否退出某项事物
			alabel1.setForeground(Color.gray);
			alabel2.setForeground(Color.gray);
			alabel3.setForeground(Color.gray);
			alabel4.setForeground(Color.gray);
		...
		public void mousePressed(MouseEvent arg0) {//判断鼠标悬停在哪
			if (arg0.getSource() == alabel1) {
				frame1.setVisible(false);
				new gamePanel().main();
