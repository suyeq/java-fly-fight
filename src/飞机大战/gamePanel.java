...
public class gamePanel extends JPanel {//实现游戏界面，继承Jpanl类
	...
	JFrame app = new JFrame("GameFrame");//建一个新的框

	public void main() {
		//t1.start();
		//t2.start();
		gamePanel game = new gamePanel();//new 该类对象
		...
		app.addWindowListener(new WindowAdapter() {//监听是否判断退出
		...
		game.setPreferredSize(new Dimension(470, 670));//设置游戏界面的宽高
		game.setOpaque(false);//允许游戏界面后的背景显示出来
		app.setSize(470, 670);
		app.setLocation(400, 50);
		app.setContentPane(bj);//加入背景
		app.add(game);//将游戏界面加入布局
	...
	public void start() {
		this.addMouseMotionListener(new MyMouseMotionListener());//监听鼠标
		this.addMouseListener(new MyMouseListener());
		Timer timer = new Timer();//设置计时器，，计时器的作用是每多少秒运行一次
